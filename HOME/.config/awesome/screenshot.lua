...
function scrot_delay()
    ...
    for key, value in ipairs(timers)  do
        items[#items+1]={tostring(value) , "scrot -d " .. value .. " " .. os.getenv("HOME") .. "/Pictures/screenshots/" .. timestamp .. ".png -e 'xclip -selection c -t image/png < $f'", "Take a screenshot of delay" }
    end
    awful.menu.new(
    {
        items = items
